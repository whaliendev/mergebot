...
#ifndef MERGEBOT_STRING_H
#define MERGEBOT_STRING_H
...
#pragma clang diagnostic pop
  ...
  size_t lastIndexOf(char ch, size_t from = npos,
    ...
    while (f >= 0) {
      if (tolower(str[f]) == ch) return from;
      --f;
    }
    return npos;
  }
  ...
  size_t indexOf(char ch, size_t from = 0,
    ...
    while (from < len) {
      if (tolower(data[from]) == ch) return from;
      ++from;
    }
    return npos;
  }

  size_t lastIndexOf(const char *ch, size_t from = npos,
    ...
    switch (len) {
      ...
      default: {
        if (cs == CaseSensitive) return mString.rfind(ch, from, len);
        if (from == npos) from = mString.size() - 1;
  ...
  size_t indexOf(const char *ch, size_t from = 0,
    ...
    switch (len) {
      ...
      default: {
        if (cs == CaseSensitive) return mString.find(ch, from, len);
  ...
  void truncate(size_t len) {
    if (mString.size() > len) mString.resize(len);
  }
  ...
  void push_back(char ch) { mString += ch; }

  void push_back(const String &ba) { mString.append(ba); }

  void push_back(const char *str, size_t len = npos) {
    if (len == npos) len = strlen(str);
    if (len > 0) mString.append(str, len);
  }
  ...
  bool endsWith(const char *str, size_t len = npos,
    ...
    const size_t s = mString.size();
    if (s >= len) {
      return (cs == CaseInsensitive ? !strncasecmp(str, c_str() + s - len, len)
                                    : !strncmp(str, c_str() + s - len, len));
    }
    return false;
  }
  ...
  bool startsWith(const char *str, size_t len = npos,
    ...
    if (len == npos) len = strlen(str);
    if (s >= len) {
      return (cs == CaseInsensitive ? !strncasecmp(str, c_str(), len)
                                    : !strncmp(str, c_str(), len));
    }
    return false;
  }
  ...
  String mid(size_t from, size_t l = npos) const {
    if (l == npos) l = size() - from;
    if (from == 0 && l == size()) return *this;
...
#pragma clang diagnostic pop
  ...
  static String join(const List<String> &list, const String &sep) {
    ...
    for (size_t i = 0; i < count; ++i) size += list.at(i).size();
    ret.reserve(size);
    for (size_t i = 0; i < count; ++i) {
      const String &b = list.at(i);
      ret.append(b);
      if (sepSize && i + 1 < list.size()) ret.append(sep);
    }
    return ret;
  }
