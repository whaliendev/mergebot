...
#ifndef MERGEBOT_MAP_H
#define MERGEBOT_MAP_H
...
class Map : public std::map<Key, Value, Compare> {
 ...
 public:
  Map() {}
  Map(const Map<Key, Value, Compare> &other) : Base(other) {}
...
#pragma clang diagnostic pop
  ...
  bool remove(const Key &t, Value *val = 0) {
    typename std::multimap<Key, Value, Compare>::iterator it =
        std::multimap<Key, Value, Compare>::find(t);
    if (it != std::multimap<Key, Value, Compare>::end()) {
      if (val) *val = it->second;
  ...
  size_t remove(std::function<bool(const Key &key)> match) {
    size_t ret = 0;
    typename std::multimap<Key, Value, Compare>::iterator it =
        std::multimap<Key, Value, Compare>::begin();
