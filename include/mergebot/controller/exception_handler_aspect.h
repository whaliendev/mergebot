...
class ExceptionHandlerAspect {
 ...
 public:
  ...
  /// \return Most of the time, the return value's type is `crow::json::wvalue`.
  /// Default constructed wvalue and wvalue initialized with nullptr or {} has
  /// the same internal type `crow::json::type::Object`. However, nullptr or {}
  /// is a valid response, while default constructed wvalue is not. Thus, the
  /// default constructed wvalue should be modified to a form we can distinguish
  /// these two situations.
  /// @see{src/server/utility.h}
  template <typename... Args>
  auto operator()(Args&&... args)
      -> decltype(func_(std::forward<Args>(args)...)) {
    static_assert(std::is_default_constructible<decltype(func_(
                      std::forward<Args>(args)...))>::value,
                  "the call return value of function passed in must be default "
                  "constructible");
    decltype(func_(std::forward<Args>(args)...)) res;
    bool err = true;
    try {
    ...
    // {} or some object meaningful
    if (err) {
      return crow::json::wvalue({{"error", "true"}});
    } else {
      return res;
    }
  }
