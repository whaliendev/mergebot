...
#ifndef MERGEBOT_LIST_H
#define MERGEBOT_LIST_H
...
class List : public std::vector<T> {
 ...
 public:
  ...
  template <typename CompatibleType>
  explicit List(std::vector<CompatibleType> &&other) : Base(other.size(), T()) {
    const size_t len = other.size();
    for (size_t i = 0; i < len; ++i) {
  ...
  void push_front(const T &t) { Base::insert(Base::begin(), t); }

  void push_front(T &&t) { Base::insert(Base::begin(), std::forward<T &&>(t)); }
  ...
  void append(const List<T> &t) {
    ...
    reserve(size() + len);
    for (size_t i = 0; i < len; ++i) Base::push_back(t.at(i));
  }
...
#pragma clang diagnostic pop
  ...
  size_t lastIndexOf(const T &t, int from = -1) const {
    ...
    if (from >= 0) {
      ...
      while (needle != haystack) {
        if (*--needle == t) return needle - haystack;
      }
    }
    return npos;
  }
  ...
  bool startsWith(const List<T> &t) const {
    ...
    for (size_t i = 0; i < t.size(); ++i) {
      if (Base::at(i) != t.Base::at(i)) return false;
    }
    return true;
  }
