...
void PostProject(const crow::request& req, crow::response& res) {
  ...
  auto internalPostProject = ExceptionHandlerAspect<CReqMResFuncType>(__doPostProject, res);
  auto rv = internalPostProject(req, res);
  if (rv.keys().size()) ResultVOUtil::return_success(res, rv);
}

void PostMergeScenario(const crow::request& req, crow::response& res) { ResultVO rv; }
